...
const generateClients = (
  ...
  Array.from({ length: 5 }, (_, i) => ({
    ...
    inactivityReason:
      STATUSES[i % STATUSES.length] === "inactive" ? "No response for 3 months" : null,
    archiveRequest: { requestedAt: null, requestedBy: null, reason: null },
    notes:
      i % 2 === 0
        ? [
            {
              content: "Key account — contact before end of quarter",
...
const seed = async () => {
  ...
  const advPOM = await User.create({
    firstName: "Piotr",
    lastName: "Doradca",
    email: "advisor.pom@seller.com",
    password: "password123",
    role: "advisor",
    grade: 3,
    position: advPOMPos._id,
    mustChangePassword: true,
    isActive: true,
    createdBy: deputy1._id,
  });
  const advWAR = await User.create({
    firstName: "Anna",
    lastName: "Doradca",
    email: "advisor.war@seller.com",
    password: "password123",
    role: "advisor",
    grade: 2,
    position: advWARPos._id,
    mustChangePassword: true,
    isActive: true,
    createdBy: deputy1._id,
  });
  const advMAL = await User.create({
    firstName: "Zofia",
    lastName: "Doradca",
    email: "advisor.mal@seller.com",
    password: "password123",
    role: "advisor",
    grade: 4,
    position: advMALPos._id,
    mustChangePassword: true,
    isActive: true,
    createdBy: deputy2._id,
  });
  const advSLA = await User.create({
    firstName: "Marek",
    lastName: "Doradca",
    email: "advisor.sla@seller.com",
    password: "password123",
    role: "advisor",
    grade: 3,
    position: advSLAPos._id,
    mustChangePassword: true,
    isActive: true,
    createdBy: deputy2._id,
  });
  ...
  advPOMPos.currentHolder = advPOM._id;
  await advPOMPos.save();
  advWARPos.currentHolder = advWAR._id;
  await advWARPos.save();
  advMALPos.currentHolder = advMAL._id;
  await advMALPos.save();
  advSLAPos.currentHolder = advSLA._id;
  await advSLAPos.save();
  ...
  superNorth.deputy = deputy1Pos._id;
  await superNorth.save();
  superSouth.deputy = deputy2Pos._id;
  await superSouth.save();
